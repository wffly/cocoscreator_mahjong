...
//发送行为(有行为推送行为，还可以做清空所有人的行为)
function sendOperations(session,self,game,seatData,pai) {
    ...
    if(hasOperations(seatData)){
        ...
        //如果可以有操作，则进行操作
        self.app.rpc.channel.ChannelRemote.backMessage(session,'game_action_push',data,seatData.userId,serverId,function () {
            data.si = seatData.seatIndex;
        });
    }
    else{
        self.app.rpc.channel.ChannelRemote.backMessage(session,'game_action_push',{},seatData.userId,serverId,commonFunction);
    }
}


//移动到下一个人(seatIndex没有的话就是向下家发牌，有的话就是往nextSeat发牌)
function moveToNextUser(game,nextSeat){
    console.log("到达moveToNextUser");
    game.fangpaoshumu = 0;
    //找到下一个没有和牌的玩家
    if(nextSeat == null){
        while(true){
...
//用户摸牌
function doUserMoPai(session,self,game){
    console.log("到达摸牌");
    ...
    //牌摸完了，结束
    if(pai == -1){
        doGameOver(session,self,game,turnSeat.userId);
        return;
    }
    else{
        var numOfMJ = game.mahjongs.length - game.currentIndex;
        for(var i=0;i<game.gameSeats.length;i++){
            self.app.rpc.channel.ChannelRemote.backMessage(session,'mj_count_push',numOfMJ,game.gameSeats[i].userId,serverId,function () {
    ...
    self.app.rpc.channel.ChannelRemote.backMessage(session,'game_mopai_push',pai,turnSeat.userId,serverId,function () {
        console.log("摸牌已通知");
        //检查是否可以暗杠或者胡
        //检查胡，直杠，弯杠
        ...
        self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_chupai_push',turnSeat.userId,roomId,function () {
            console.log("已通知下个人出牌");
        });
        //通知玩家做对应操作
        sendOperations(session,self,game,turnSeat,game.chuPai);
    });
}
...
function calculateResult(game,roomInfo){
    ...
    var isNeedChaDaJiao = needChaDaJiao(game);
    if(isNeedChaDaJiao){
        chaJiao(game);
    }
        ...
        if(isTinged(sd)){
            ...
            for(var a = 0; a < sd.actions.length; ++a){
                ...
                if(ac.type == "fanggang"){
                    ...
                    if(isNeedChaDaJiao && (ts.hued) == false && (isTinged(ts) == false)){
                        ac.state = "nop";
                    }
                }
...
//游戏结束逻辑
function doGameOver(session,self,game,userId,forceEnd){
    var roomId = roomMgr.getUserRoom(userId);
    if(roomId == null){
        return;
    }
    ...
    var fnNoticeResult = function(isEnd){
        ...
        self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_over_push',{results:results,endinfo:endinfo},roomId,function () {
           console.log("已推送游戏结束");
        });
    ...
    if(game != null){
        ...
        if(old != roomInfo.nextButton){
            self.app.rpc.dao.RoomDao.update_next_button(session,roomId,roomInfo.nextButton,function () {
    ...
    if(forceEnd || game == null){
        ...
        //保存游戏
        self.app.rpc.dao.GameDao.create_game(session,game.roomInfo.uuid,game.gameIndex,game.baseInfoJson,function () {
            //记录游戏结果
            self.app.rpc.dao.GameDao.update_game_result(session,roomInfo.uuid,game.gameIndex,dbresult,function () {
                var str = JSON.stringify(game.actionList);
                //更新游戏行为记录
                self.app.rpc.dao.GameDao.update_game_action_records(session,roomInfo.uuid,game.gameIndex,str,function () {
                    //保存游戏局数
                    self.app.rpc.dao.GameDao.update_num_of_turns(session,roomId,roomInfo.numOfGames,function () {
                        if(roomInfo.numOfGames == 1){
                            var cost = 2;
                            ...
                            //减少房卡
                            self.app.rpc.dao.GameDao.cost_gems(session,game.gameSeats[0].userId,cost,function () {
...
//xzdd的设置准备
exports.setReady = function(session,self,userId,callback){
    var roomId = roomMgr.getUserRoom(userId);
    if(roomId == null){
        return;
    }
    ...
    if(game == null){
        if(roomInfo.seats.length == 4){
            ...
            exports.begin(session,self,roomId);
        }
    }
    else{
        ...
        sendOperations(session,self,game,seatData,game.chuPai);
    }
}
...
//4人到齐开始游戏
exports.begin = function(session,self,roomId) {
    console.log("到达gamemgr_xzdd begin");
    var roomInfo = roomMgr.getRoom(roomId);
    if(roomInfo == null){
        return;
    }
    ...
    var sendMsg = function () {
        if(i<4){
            ...
            console.log(s);
            //通知玩家手牌
            console.log("userId:"+s.userId+"  "+"serverId:"+serverId);
            self.app.rpc.channel.ChannelRemote.backMessage(session,'game_holds_push',game.gameSeats[i].holds,s.userId,serverId,function() {
                console.log("已通知玩家个人手牌");
                self.app.rpc.channel.ChannelRemote.backMessage(session,'mj_count_push',numOfMJ,s.userId,serverId,function(){
                    console.log("已通知还剩多少张牌");
                    self.app.rpc.channel.ChannelRemote.backMessage(session,'game_num_push',{numOfGames:roomInfo.numOfGames},s.userId,serverId,function(){
                        console.log("已通知还剩多少局");
                        self.app.rpc.channel.ChannelRemote.backMessage(session,'game_begin_push',{button:game.button},s.userId,serverId,function(){
                            console.log("已通知游戏开始");
                            if(huansanzhang == true){
                                game.state = "huanpai";
                                //通知准备换牌
                                self.app.rpc.channel.ChannelRemote.backMessage(session,'game_huanpai_push',{},s.userId,serverId,function(){
                                    console.log("已通知准备换牌");
                                });
                            }
                            else{
                                game.state = "dingque";
                                //通知准备定缺
                                self.app.rpc.channel.ChannelRemote.backMessage(session,'game_dingque_push',{},s.userId,serverId,function(){
                                    console.log("已通知定缺");
                                });
                            }
                            i++;
...
//换三张
exports.huanSanZhang = function(session,self,p1,p2,p3){
    console.log("到达gamemgr_xzdd huanSanZhang");
    ...
    console.log("userId:"+userId+" roomId:"+roomId+" serverId:"+serverId);
    var seatData = gameSeatsOfUsers[userId];
    if(seatData == null){
        console.log("can't find user game data.userId:"+userId);
        return;
    }

    var game = seatData.game;
    if(game.state != "huanpai"){
        console.log("can't recv huansanzhang when game.state == " + game.state+" userId:"+userId);
        return;
    }

    if(seatData.huanpais != null){
        console.log("player has done this action."+" userId:"+userId);
        return;
    }
    ...
    //推送手牌
    self.app.rpc.channel.ChannelRemote.backMessage(session,'game_holds_push',seatData.holds,userId,serverId,commonFunction);
    ...
    for(var i = 0; i < game.gameSeats.length; ++i){
        ...
        if(sd == seatData){
            ...
            self.app.rpc.channel.ChannelRemote.backMessage(session,'huanpai_notify',rd,sd.userId,serverId,commonFunction);
        }
        else{
            var rd = {
                si:seatData.userId,
    ...
    for(var i = 0; i < s.length; ++i){
        ...
        self.app.rpc.channel.ChannelRemote.backMessage(session,'game_huanpai_over_push',rd,userId,serverId,commonFunction);
        self.app.rpc.channel.ChannelRemote.backMessage(session,'game_holds_push',s[i].holds,userId,serverId,commonFunction);
        //通知准备定缺
        self.app.rpc.channel.ChannelRemote.backMessage(session,'game_dingque_push',{},userId,serverId,commonFunction);
    }
};

//定缺
exports.dingQue = function(session,self,userId,type){
    console.log("到达gamemgr_xzdd dingQue");
    ...
    if(game.numOfQue == 4){
        ...
        self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_dingque_finish_push',arr,roomId,function (){
            self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_playing_push',{},roomId,function () {
                //进行听牌检查
                //进行听牌检查
                for(var i = 0; i < game.gameSeats.length; ++i){
                    var duoyu = -1;
        ...
        self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_dingque_notify_push',userId,roomId,function () {
            console.log("已通知定缺状态");
        });
    }
};
...
//出牌
exports.chuPai = function(session,self,pai){
    var userId = session.uid;
    var roomId = session.get('roomId');
    var serverId = self.app.get('serverId');
    var commonFunction = function () {
    ...
    console.log("------gamemgr_xzdd line:1573 pai:"+pai+"------");
    var seatData = gameSeatsOfUsers[userId];
    if(seatData == null){
        console.log("------can't find user game data.------");
        return;
    ...
    self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_chupai_notify_push',{
        userId:seatData.userId,
        pai:pai
    },roomId,commonFunction);
    ...
    for(var i = 0; i < game.gameSeats.length; ++i){
        ...
        if(seatData.lastFangGangSeat == -1){
            if(ddd.canHu && ddd.guoHuFan >= 0 && ddd.tingMap[pai].fan <= ddd.guoHuFan){
                ...
                ddd.canHu = false;
                self.app.rpc.channel.ChannelRemote.backMessage(session,'guohu_push',ddd.userId,serverId,commonFunction);
            }
        }
        ...
        if(hasOperations(ddd)){
            hasActions = true;
            sendOperations(session,self,game,ddd,game.chuPai);
        }
    }
    ...
    if(!hasActions){
        setTimeout(function(){
            self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'guo_notify_push',{userId:seatData.userId,pai:game.chuPai},roomId,function () {
                seatData.folds.push(game.chuPai);
                game.chuPai = -1;
                moveToNextUser(game);
                doUserMoPai(session,self,game);
            });
        },500);
    }
...
//碰
exports.peng = function(session,self,userId){
    var seatData = gameSeatsOfUsers[userId];
    if(seatData == null){
        console.log("can't find user game data.");
        return;
    ...
    var roomId = session.get('roomId');
    //广播通知其它玩家
    self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'peng_notify_push',{userid:seatData.userId,pai:pai},roomId,function () {
        //碰的玩家打牌
        moveToNextUser(game,seatData.seatIndex);
        //广播通知玩家出牌方
        seatData.canChuPai = true;
        self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'game_chupai_push',seatData.userId,roomId,function () {
            console.log("已通知玩家出牌")
        });
    });
};
...
function checkCanQiangGang(game,turnSeat,seatData,pai){
    ...
    for(var i = 0; i < game.gameSeats.length; ++i){
        ...
        checkCanHu(game,ddd,pai);
        if(ddd.canHu){
            sendOperations(session,self,game,ddd,pai);
            hasActions = true;
        }
    }
    if(hasActions){
...
//杠的逻辑
function doGang(session,self,game,turnSeat,seatData,gangtype,numOfCnt,pai){
    var seatIndex = seatData.seatIndex;
    ...
    checkCanTingPai(game,seatData);
    //通知其他玩家，有人杠了牌
    self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'gang_notify_push',{userid:seatData.userId,pai:pai,gangtype:gangtype},roomId,function () {
...
//杠
exports.gang = function(session,self,pai){
    console.log("到达gamemgr_xzdd gang");
    console.log(pai);
    var userId = session.uid;
    var roomId = session.get('roomId');
    var serverId = self.app.get('serverId');
    ...
    clearAllOptions(game);
    seatData.canChuPai = false;
    self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'hangang_notify_push',seatIndex,roomId,function () {
        console.log("已喊杠");
    });
    ...
    doGang(session,self,game,turnSeat,seatData,gangtype,numOfCnt,pai);
};

//过
exports.guo = function(session,self){
    console.log("到达gamemgr_xzdd guo");
    var userId = session.uid;
    var serverId = self.app.get('serverId');
    var roomId = session.get('roomId');
    var seatData = gameSeatsOfUsers[userId];
    ...
    self.app.rpc.channel.ChannelRemote.backMessage(session,'guo_result',{},seatData.userId,serverId,function () {
        ...
        if(game.chuPai >= 0){
            ...
            self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'guo_notify_push',{userId:uid,pai:game.chuPai},roomId,function () {
...
//胡
exports.hu = function(session,self){
    console.log("到达gamemgr_xzdd hu");
    var userId = session.uid;
    var roomId = session.get('roomId');
    var seatData = gameSeatsOfUsers[userId];
    ...
    clearAllOptions(game,seatData);
    // -- 通知前端，有人和牌了
    self.app.rpc.channel.ChannelRemote.pushMessageInRoom(session,'hu_push',{seatindex:seatIndex,iszimo:isZimo,hupai:notify},roomId,function () {
    ...
    // -- 和了三家
    if(numOfHued == 3){
        doGameOver(session,self,game,seatData.userId);
        return;
    }
    ...
    for(var i = 0; i < game.gameSeats.length; ++i){
        ...
        ddd.canGang = false;
        ddd.canChuPai = false;
        sendOperations(session,self,game,ddd,hupai);
    }
...
exports.doDissolve = function(roomId){
    ...
    var game = games[roomId];
    doGameOver(session,self,game,roomInfo.seats[0].userId,true);
};
